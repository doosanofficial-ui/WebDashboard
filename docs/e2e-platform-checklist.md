...
- [ ] 로드뷰 렌더링 정상
- [ ] GPS 이동 시 지도/로드뷰 동기화
- 주의: 본 섹션은 맵 기능 릴리스 기준이며, iOS 백그라운드 위치 검증(P3-3)의 블로커는 아님
...
- [ ] **큐 flush 완료** (Y/N)
- [ ] **GPS 신선도 지연** ≤ 300초
- [ ] BG PASS에는 동일 설치 빌드의 네이티브 수집, 30분 background 구간 및 원본/서버 로그 대조 증거가 필수. 지도 오류는 이 BG 시험만 비차단이며 전체 맵 기능 미완료는 별도 유지
