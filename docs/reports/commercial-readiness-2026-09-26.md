...
Current native development version: `0.9.0` (build `1`).
...
| --- | --- | --- |
| Swift Core tests | PASS, 75 tests | `mobile-ios/scripts/verify.sh build`; `0.9.0` artifact `/tmp/telemetry-ios-verify.gnDrF6` |
...
| Native app build after hardening | PASS | XcodeGen-generated `0.9.0` project, Xcode 26.3/iOS 26.2 Simulator SDK; `/tmp/telemetry-ios-verify.gnDrF6` |
...
| Native Reliability | PASS, Core + native app + Linux/Windows contracts | PR #23 run `36258712160`; native-app job generated XcodeGen project and built the iOS target |
...
| iPhone 17 physical build | PASS (SDK boundary) | Fresh `0.9.0` Personal Team build `/tmp/telemetry-ios-device-condition.GbIvr0` with Xcode 26.3/iOS 26.2 SDK; not an iOS 27 SDK result |
| iPhone 17 physical install | PASS | `0.9.0` build `1` installed by `devicectl` |
| iPhone 17 physical launch | BLOCKED BY DEVICE LOCK | `verify_device.sh` returned exit `12`; log `/tmp/telemetry-v090-device-check.log`; unlock the iPhone and rerun |
