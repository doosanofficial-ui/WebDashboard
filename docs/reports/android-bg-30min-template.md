...
이 템플릿은 Android 앱의 백그라운드 GPS uplink 30분 시나리오를 반복 실행할 때 비교 가능한 보고서를 생성하기 위한 양식이다.
...
- 총 실행 시간: 30분 (± 1분)
- 화면 상태: 앱 백그라운드 + 화면 OFF 유지
- GPS 모드: Foreground Service(FGS) + Background location 권한
...
- [ ] `source` 컬럼이 `mobile` 또는 `android-rn`인지 확인
